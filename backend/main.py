...
if PIECE_NAME:
    ...
    PATH_LIVE_WAV = os.path.join(
        "data", "audio", PIECE_NAME, f"live_{LIVE_TEMPO}bpm.wav"
    )
...
if PATH_ALIGNMENT_CSV and (not ALIGN_COL_LIVE or not ALIGN_COL_REF):
    raise ValueError(
        "If specifying 'path_alignment_csv', must also specify 'align_col_ref', 'align_col_live'"
    )
...
live_buffer = AudioBuffer(
    max_duration=MAX_DURATION, sample_rate=SAMPLE_RATE, channels=1
)

# Create a ScoreFollower instance to track the soloist
score_follower = ScoreFollower(
    ref_filename=PATH_REF_WAV,
    c=C,
...
def step(data) -> int:
    ...
    )  # log soloist time for error analysis
    estimated_times.append(estimated_time)  # log estimated time for error analysis

    position = (
        estimated_time / 60 * REF_TEMPO
    )  # convert estimated time (seconds) to position in piece (beats)
    return position
...
def callback(in_data, frame_count, time_info, status):
    ...
    if USE_MIC:  # If using microphone input
        ...
        data = live_audio[
            :, source_index : source_index + frame_count
        ]  # get audio data from source audio
        source_index += frame_count  # update source index

    live_buffer.write(
        data
    )  # write soloist audio to buffer so it can be saved to a WAV file later
    position = step(data)
    ...
    return (
        SOLO_VOLUME_MULTIPLIER * data,
        pyaudio.paContinue,
    )  # Output the solo to the speakers. The accompaniment is already being played by the MidiPerformance instance.
...
if SKIP_PLAYBACK:
    ...
    while source_index < live_audio.shape[-1]:
        data = live_audio[
            :, source_index : source_index + WIN_LENGTH
        ]  # get audio data from source audio
    ...
    performance = MidiPerformance(
        midi_file_path=PATH_REF_MIDI,
        tempo=REF_TEMPO,
        instrument_index=ACCOMP_INSTR_INDEX,
        program_number=ACCOMP_PROGAM_NUMBER,
        soundfont_path=PATH_SOUNDFONT,
    )
