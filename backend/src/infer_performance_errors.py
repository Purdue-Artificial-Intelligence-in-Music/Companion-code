...
def plot_f0(waveform, yin_frame_len):
    ...
    plt.plot(
        sample_indices_yin[valid_yin],
        f0_yin_midi[valid_yin],
        color="purple",
        linewidth=1,
        label="YIN $f_0$ (MIDI)",
    )
    ...
    plt.plot(
        sample_indices_pyin[valid_pyin],
        f0_pyin_midi[valid_pyin],
        color="orange",
        linewidth=1,
        label="pYIN $f_0$ (MIDI)",
        alpha=0.7,
    ...
    plt.yticks(
        np.arange(int(np.nanmin(f0_yin_midi)), int(np.nanmax(f0_yin_midi)) + 1, 1)
    )
