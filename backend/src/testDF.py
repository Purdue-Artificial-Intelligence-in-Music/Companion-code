...
def x_to_y(num, xData, yData):
    ...
    slope = (yData[location + 1] - yData[location]) / (
        xData[location + 1] - xData[location]
    )
    if slope == 0:
        return yData[location]
...
def main():
    ...
    for i in range(0, source_audio.shape[-1], 4096):
        ...
        ref_index_list.append(
            score_follower.otw.ref_index
            * score_follower.win_length
            / score_follower.sample_rate
        )
        live_index_list.append(
            score_follower.otw.live_index
            * score_follower.win_length
            / score_follower.sample_rate
        )
    ...
    for time in testDF[
        "baseline_beat"
    ]:  # testDF['baseline_time_s] - expected time of each note in the baseline
        idx = np.argmin(
            np.abs(ref_time - time)
        )  # find closest time in warping path (closest point in ref_time, which is the ref time sequence that DTW aligned)
        predicted_source_times.append(
            live_time[idx]
        )  # append corresponding live time as the predicted rubato time
    ...
    newDF["rubato devation"] = (
        newDF["predicted rubato time"] - newDF["actual rubato time"]
    )
