...
def calculate_accompaniment_error(df: pd.DataFrame, estimated_times: np.ndarray,
                                  accompanist_times: np.ndarray) -> pd.DataFrame:
    """
    ...
    result = df.assign(accompaniment=nearest_acc,
                       accompaniment_error=accompaniment_error)
    return result
