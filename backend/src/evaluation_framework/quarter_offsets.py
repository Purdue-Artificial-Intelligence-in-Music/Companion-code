...
# Define the reference tempo for quarter note conversion
REFERENCE_TEMPO = 100  # BPM - Sonata's tempo is around 100 BPM
...
def extract_notes(midi_path):
    ...
    # Ensure we have the initial tempo at position 0
    ...
    for track_idx, track in enumerate(midi.tracks):
        absolute_tick = 0
        active_notes = {}  # Dictionary of active notes: (pitch, channel) -> start_info
        ...
        for msg in track:
            ...
            # Handle note off events (or note on with velocity 0)
                ...
                if note_id in active_notes:
                    ...
                    if duration_ticks > 0:  # Skip zero-duration notes
                        # Convert to seconds using tempo map
                        ...
                        # Convert to original beat units (6/8 time)
                        ...
                        # Convert to quarter note beats (in 6/8 time, 1 measure = 2 quarter notes)
...
def tick_to_seconds(tick, tempo_changes, ticks_per_beat):
    """
    Convert a tick position to seconds, accounting for tempo changes.
    
    Args:
        tick: Tick position
        tempo_changes: List of (tick, tempo) pairs
    ...
    for change_tick, tempo in tempo_changes:
        if change_tick < tick:
            # Add time for the segment from last_tick to change_tick
...
def compare_midi_files(baseline_path, accelerando_path, output_path):
    """
    Compare notes between baseline and accelerando MIDI files.
    
    Args:
        baseline_path: Path to the baseline MIDI file
        accelerando_path: Path to the accelerando MIDI file
        output_path: Path to save the CSV output
        
    Returns:
        DataFrame with note offset comparisons
    """
    # Extract notes from both files
    baseline_data = extract_notes(baseline_path)
    accelerando_data = extract_notes(accelerando_path)
    
    # Initialize a list to store all note comparison data
    comparison_data = []
    global_note_id = 0
    
    # Process all notes in baseline data
    for baseline_note in baseline_data['notes']:
        # Find the corresponding note in accelerando data
        matched_notes = [
        ...
        record = {
            ...
            # Calculate percentage of time saving
...
def create_simplified_quarter_note_mapping(comparison_df, output_path, reference_tempo=REFERENCE_TEMPO):
    """
    Create a simplified mapping with quarter note beats using a specific reference tempo.
    
    Args:
        comparison_df: DataFrame with note comparisons
        output_path: Path to save the simplified CSV
        reference_tempo: Reference tempo for quarter note beats (BPM)
        
    Returns:
        DataFrame with simplified mapping
    """
    # Calculate the duration of one quarter note at the reference tempo (in seconds)
    ...
    simplified = pd.DataFrame({
        ...
        'Transformed Quarter Note Beat': comparison_df['accelerando_quarter_note_beat'],
        'Original Time (s)': comparison_df['original_seconds'],
        'Transformed Time (s)': comparison_df['accelerando_seconds'],
...
def main():
    """Main function to extract offsets and create mappings."""
    # File paths
    baseline_path = "../altered_urmp/02_Sonata_vn_vn/baseline.mid"
    accelerando_path = "accelerando.mid"
    comparison_path = "note_offsets.csv"
    simplified_path = "quarter_note_mapping.csv"
    
    # Copy the baseline MIDI to our folder
    import shutil
    shutil.copy2(baseline_path, "baseline.mid")
    print(f"Copied baseline MIDI to: baseline.mid")
    
    # Compare MIDI files and extract note offsets
    comparison_df = compare_midi_files(baseline_path, accelerando_path, comparison_path)
    
    # Create simplified mapping with quarter note beats
