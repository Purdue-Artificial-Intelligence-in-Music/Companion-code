...
def apply_smooth_tempo_curve(
    midi_file,
    output_file="smooth_tempo_curve.mid",
    curve_type="sine",
    ...
    for i in range(num_tempo_changes + 1):
        ...
        new_tempo = int(
            original_tempo / tempo_factor
        )  # Slower tempo = higher tempo value
...
def apply_deliberate_pauses(midi_file, pause_seconds=0.75):
    ...
    new_midi = MidiFile()
    pause_ticks = mido.second2tick(
        pause_seconds, midi_file.ticks_per_beat, bpm2tempo(120)
    )
    ...
    for track in midi_file.tracks:
        ...
        for msg in track:
            if (
                msg.type == "note_on"
                and ticks_elapsed % (midi_file.ticks_per_beat * 4) == 0
            ):
                new_track.append(
                    Message(
                        "note_off",
    ...
    for track in midi_file.tracks:
        ...
        for msg in track:
            if msg.type in ["note_on", "note_off"]:
                variation = int(
                    msg.time * (random.uniform(-rubato_percent, rubato_percent) / 100)
                )
...
def plot_tempo_curve(
    curve_type, max_tempo_change=0.3, curve_periods=2, output_file="tempo_curve.png"
):
    """
...
def process_urmp_piece(
    piece_folder, curve_type="sine", max_tempo_change=0.3, curve_periods=2
):
    """
