...
from src.accompaniment_error import (
    load_data,
    find_nearest,
    calculate_accompaniment_error,
)
...
class TestAccompanimentFunctions(unittest.TestCase):
    ...
    def test_load_data_missing_columns(self, mock_read_csv):
        ...
        data = {"measure": [1, 2, 3, 4], "live": [0.52, 1.08, 1.55, 2.10]}
        mock_read_csv.return_value = pd.DataFrame(data)
        ...
        self.assertTrue(
            "Missing one or more required columns" in str(context.exception)
        )
    ...
    def test_calculate_accompaniment_error(self):
        ...
        result_df = calculate_accompaniment_error(
            df, estimated_times, accompanist_times
        )
