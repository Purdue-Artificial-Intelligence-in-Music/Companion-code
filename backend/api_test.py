...
try:
    ...
    if start_response.status_code == 200:
        ...
        print(
            f"Failed to start session. Status code: {start_response.status_code}, Message: {start_response.text}"
        )
        exit()
...
try:
    ...
    if scores_response.status_code == 200:
        ...
        print(
            f"Failed to get music scores. Status code: {scores_response.status_code}, Message: {scores_response.text}"
        )
except requests.RequestException as e:
...
try:
    ...
    if score_response.status_code == 200:
        ...
        print(
            f"Failed to download music score. Status code: {score_response.status_code}, Message: {score_response.text}"
        )
except requests.RequestException as e:
...
try:
    ...
    if audio_response.status_code == 200:
        ...
        print(
            f"Failed to synthesize audio. Status code: {audio_response.status_code}, Message: {audio_response.text}"
        )
except requests.RequestException as e:
...
source_file = os.path.join(
    "data",
    "audio",
    score_filename.replace(".musicxml", ""),
    "live",
    "constant_tempo.wav",
)
...
for i in range(0, source_audio.shape[-1], 8192):
    ...
    payload = {"frames": frames, "timestamp": accompanist_time}
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"Soloist time: {soloist_time:.2f}, Estimated time: {estimated_time:.2f}, Accompanist time: {accompanist_time:.2f}, Playback rate: {playback_rate:.2f}"
            )
        else:
