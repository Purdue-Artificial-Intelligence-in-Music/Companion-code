...
const handleResponse = async (response: Response) => {
  if (response.ok) {
    //doing below checks because sometimes getting a string sometimes getting a json.
...
export const synthesizeAudio = async (
  ...
  filename: string,
  tempo: number,
): Promise<{ uri: string; sample_rate: number }> => {
  console.log(sessionToken, filename, tempo);
  const response = await fetch(
    `${BASE_URL}/synthesize-audio/${filename}/${tempo}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
  ...
  return { uri, sample_rate: parseInt(sampleRate || "44100") };
};
