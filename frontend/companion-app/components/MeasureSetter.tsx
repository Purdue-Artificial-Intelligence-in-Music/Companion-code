...
export function MeasureSetBox({
  ...
  cursorRef: MutableRefObject<Cursor | null>;
  ...
  BPM: number;
  TSD: number; // Beats per minute and time signature denominator, respectively
}) {
  ...
  useEffect(() => {
    if (needsReset) {
      ...
      while (
        cursorRef.current?.Iterator.CurrentMeasureIndex !== undefined &&
        cursorRef.current?.Iterator.CurrentMeasureIndex < desired_index
      ) {
